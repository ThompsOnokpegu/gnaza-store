<div class="lg:pl-20">
    <div class="mb-6 ">
        <h2 class="max-w-xl mb-3 text-3xl font-ridley font-normal uppercase leading-normal tracking-wide md:text-3xl">
            {{ $product->name }}
        </h2>
    ...
    <div class="mb-6 ">
        @if(session()->has('qty-required'))
            <div class="mb-3 text-gz-danger bg-gz-danger-bg font-semibold rounded-md py-3 px-5">
                
                <p>{{ session('qty-required') }}</p>
            </div>
        @endif

        @if(session()->has('added'))
            <div class="mb-3 text-gz-success bg-gz-success-bg font-semibold rounded-md py-3 px-5">
                <p>{{ session('added') }}</p>
    ...
    <div class="flex flex-wrap items-center mb-6">
        ...
        <div class="mb-4 mr-4 lg:mb-0">
            <div class="w-28">
                <div class="relative flex flex-row w-full h-10 bg-transparent rounded-lg">
                    <button wire:click="decrement()" class="w-20 h-full text-gz-black bg-white outline-none cursor-pointer hover:text-white hover:bg-black">
                    <span class="m-auto text-2xl font-thin">-</span>
                    </button>
                
                    <input type="number" value="{{ $quantity }}" class="border-none flex items-center w-full font-semibold text-center text-black placeholder-gz-black-700 bg-white outline-none focus:outline-none text-md hover:text-black" placeholder="1">
                    <button wire:click="increment()" class="w-20 h-full text-gz-black bg-white outline-none cursor-pointer hover:text-white hover:bg-black">
                    <span class="m-auto text-2xl font-thin">+</span>
                    </button>
                </div>
            </div>
        ...
        {{-- <div class="mb-4 lg:mb-0">
            <button class="flex items-center justify-center w-full h-10 p-2 mr-4 border border-gz-black-200 lg:w-11 hover:text-gray-50">
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-gz-black-200 hover:stroke-gz-black-200" width="16" height="16" fill="currentColor" class=" bi bi-heart" viewBox="0 0 16 16">
                    <path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z"></path>
                </svg>
            </button>
        </div> --}}
        <a href="#" wire:click.prevent="addToCart" class="w-full px-4 py-3 text-center text-black bg-white border border-black hover:text-white hover:bg-black lg:w-1/2">
        Add to cart
        </a>
    </div>
    <div class="flex gap-4 mb-6">
        <a wire:navigate href="{{ route('cart') }}" class="w-full px-4 py-3 text-center text-white bg-black border border-transparent hover:border-black hover:text-black hover:bg-white">
        Review Your Cart</a>
    </div>
    <div class="py-6 mb-6">
        <span class="text-base text-black">In Stock</span>
        <p class="mt-2 text-sm font-ridley text-yellow-500">Ships from Abuja
    ...
    <div class="mb-6">
        ...
        <div class="md:border-t md:border-b md:border-gz-black-500">
            <div class="p-3 lg:p-5 ">
                <div class="p-2 rounded-xl lg:p-6 bg-white font-ridley">
